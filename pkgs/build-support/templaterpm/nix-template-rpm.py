    ...
    with file(tableFile, 'r') as infile:
      for line in infile:
        ...
        if match is not None:
          if not self.tablePath.has_key(match[1]):
            self.tablePath[match[1]] = match[2]
          if not self.tableName.has_key(match[1]):
            self.tableName[match[1]] = match[3]
        else:
